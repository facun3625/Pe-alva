        ...
        <tr>
          <td style="background: white; padding: 32px; border-radius: 0 0 12px 12px;">
            ...
            <div style="margin-top: 28px; padding-top: 24px; border-top: 1px solid #f0efed; text-align: center;">
              <a href="${siteUrl}/propiedades" style="display: inline-block; background: #262522; color: white; text-decoration: none; padding: 12px 28px; border-radius: 8px; font-size: 13px; font-weight: 700;">Ver todas las propiedades</a>
            </div>
          </td>
        </tr>
        <tr>
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const alert = await prisma.propertyAlert.findUnique({ where: { id: log.alertId } });
  if (!alert) return NextResponse.json({ error: "Suscriptor no encontrado" }, { status: 404 });
  ...
  if (properties.length === 0) {
    return NextResponse.json({ error: "No hay propiedades que coincidan actualmente" }, { status: 400 });
  }
  ...
  try {
    ...
    await prisma.emailLog.create({
      data: { to: alert.email, nombre: alert.nombre, subject: asunto, alertId: alert.id },
    });
