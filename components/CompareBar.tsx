    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#262522] border-t border-white/10 shadow-2xl">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 overflow-x-auto">
          {ids.map((id) => {
            ...
            return (
              <div key={id} className="flex items-center gap-2 bg-white/10 rounded-lg px-3 py-1.5 shrink-0">
                ...
                <span className="text-white text-[12px] max-w-[140px] truncate">
                  {p?.title ?? "Cargando..."}
                </span>
                <button onClick={() => remove(id)} className="text-white/40 hover:text-white transition-colors ml-1">
                  <X size={13} />
                </button>
              </div>
            );
          ...
          {ids.length < 3 && (
            <div className="flex items-center justify-center w-12 h-9 border border-dashed border-white/20 rounded-lg text-white/25 text-lg shrink-0">
              +
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button onClick={clear} className="text-white/40 hover:text-white text-[12px] transition-colors">
            Limpiar
          </button>
          {ids.length >= 2 && (
            <button
